...
This script produces the summary of key measurements from teh experiment grouped by each subject and activity type.
...
Basically, those variables are the ean and standard deviation from the measurements from the accelerometer and gyrometer from the experiment along with the derivative features.

Then, in the last part of the script, these measurements are summarized using the average for each subject and activity